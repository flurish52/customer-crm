<template>
    <div class="bg-white rounded-xl shadow-sm border border-tertiary-light overflow-hidden ">
        ...
        <div class="hidden md:grid grid-cols-12 gap-4 px-6 py-4 bg-tertiary-light/50 border-b border-tertiary-light">
            ...
            <div class="col-span-2 font-medium text-primary-dark">Total Spent</div>
            <div class="col-span-2 text-center font-medium text-primary-dark">Jobs</div>
        ...
        >
          ...
          <span class="text-sm font-medium text-primary-dark mr-2">
            ...
            {{ customer?.overdueJobs > 0 ? `${customer.overdueJobs} overdue` : `${customer.pendingJobs} pending` }}
                ...
                <div v-if="customer.invoices && customer.invoices.length > 0" class="pt-3 border-t border-gray-100">
                    <p class="text-xs text-gray-500 uppercase font-medium mb-2">Total Spent</p>
                    <div class="flex items-center">
                        {{ totalSpendPerClient(customer) }}
                    </div>
                </div>
            </div>
        ...
        <span class="text-sm font-medium text-center text-primary-dark mr-2">
          {{ customer.jobs?.length || 0 }}
            ...
            <div class="hidden md:flex md:col-span-2 md:justify-end md:items-center md:space-x-2">
                <Link
                    :href="`/customer/${customer.id}`"
                    class="p-2 flex  rounded-lg hover:bg-tertiary-light text-primary"
                    title="View"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                         stroke="currentColor">
                ...
                <button
                    @click="editCustomer(customer)"
                    class="p-2 flex rounded-lg hover:bg-tertiary-light text-blue-500"
                    title="Edit"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                         stroke="currentColor">
...
const totalSpendPerClient = (client) =>
    (client.invoices || [])
        .filter(inv => inv.status !== 'cancelled')
        .reduce((sum, inv) =>
                sum + (inv.payments || [])
                    .filter(p => !p.is_invalid)
                    .reduce((s, p) => s + Number(p.amount_in_business_currency || 0), 0)
            , 0);

            ...
            <div class=" bg-primary-dark rounded-md px-3 py-1">
                ...
                <div class="space-y-1 text-sm bg-white w-fit px-3 rounded-md mb-2 text-primary-dark">
                    ...
                    <p>
                        ...
                        <span class="italic ">{{ item.client.email }}</span>
                    </p>
                    <p>
                        <span class="font-medium">Replying to:</span>
                        <span class="italic">{{ JSON.parse(item.business_snapshot)?.email }}</span>
                    </p>
                    <hr class="py-0.5">
                </div>
            </div>
            <div class="space-y-4 my-1">
                <div>
                    ...
                    <input
                        ...
                        :placeholder="`Quote ${item.quote_number}`"
                    />
                </div>
                <div>
                ...
                <div class="flex  w-full">
                    <p class="text-sm text-red-600 px-3 italic ">
                        Please note that the quote PDF is automatically attached to this email.
                    </p>
                </div>
            </div>
...
function confirmSend() {
    ...
    const emailBody = {
        ...
        from_name: JSON.parse(props.item?.business_snapshot).name,
        message: emailMessage.value,
        sendToEmail: props.item?.client.email,
        replyToEmail: JSON.parse(props.item?.business_snapshot).email,
    }
    axios.post(`/dashboard/user/${props.item.quote_number}/send_quote`, emailBody)
        .then(res => {
            if (res.status === 200) {
